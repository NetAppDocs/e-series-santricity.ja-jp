...
.手順
. 左側のパネルから*ホーム*を選択します。
. *Bell*アイコンをクリックし、ウィンドウの中央上部にある*Recover from _n_ problems*というラベルの付いたリンクをクリックします。
+
...
+
ファイルは、ブラウザのダウンロードフォルダに次の名前で保存されます `recoveryGuru-failureName-arrayName-yyyy-mm-dd-hh-mm-ss.html`。
