...
summary: SSD Cacheの設定を編集できます。SSD Cacheの現在の設定は、View settingsオプションからアクセスできます。 
---
...
[role="lead"]
SSD Cacheの設定を編集できます。SSD Cacheの現在の設定は、View settingsオプションからアクセスできます。

.手順
. 左側のパネルから、メニュー：Storage[Pools and volume groups]を選択します。
. 編集したいSSDキャッシュを選択し、メニュー：省略記号アイコン[設定の編集]をクリックします。
+
SSD キャッシュ設定の編集ウィンドウが表示されます。
